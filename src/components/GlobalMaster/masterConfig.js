import {
  ...
  Banknote, MapPin, Users, BookOpen, Globe, Car, Hash, Layers,
} from "lucide-react";
...
const operationCapacityRows = Array.from({ length: 14 }, (_, i) => ({
  id: String(i + 1),
...
const clearingBankRows = Array.from({ length: 14 }, (_, i) => ({
  id: String(i + 1),
...
const cityRows = Array.from({ length: 14 }, (_, i) => ({
  id: String(i + 1),
...
export const buildRowFromForm = (masterKey, formData) => {
  const config = getMasterConfig(masterKey);
  const row = { ...formData };
